//! Integration tests driving `XarArchive` through its public API.
//!
//! These build XAR byte streams and read them back, so they need nothing
//! private. Keeping them out of `src/` means the compiler enforces that: a
//! test that reaches for an internal stops building here rather than quietly
